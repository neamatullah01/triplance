  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} className={`h-3.5 w-3.5 ${i < rating ? "text-amber-400 fill-amber-400" : "text-slate-300 dark:text-slate-600"}`} />
      ))}
      <span className="ml-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300">{rating}.0</span>
    </div>
  );
}
...
export default function ReviewsPage() {
  ...
  const filtered = reviewsData.filter((r) => filter === "all" ? true : (filter === "reported" ? r.reported : true));
  const flaggedCount = reviewsData.filter((r) => r.reported).length;

  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Reviews</h1>
        ...
        {flaggedCount > 0 && (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-xl self-start sm:self-auto shrink-0">
            ...
            <span className="text-xs font-semibold text-red-700 dark:text-red-400">{flaggedCount} flagged reviews</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="relative w-full sm:w-auto sm:flex-1 sm:max-w-xs sm:ml-auto">
          ...
          <input type="text" placeholder="Search reviews..." className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900" />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800">
                <th className="px-4 sm:px-6 py-3">Reviewer</th>
                ...
                <th className="px-4 sm:px-6 py-3 hidden md:table-cell">Comment</th>
            ...
            <tbody>
              {filtered.map((review) => (
                <tr key={review.id} className={`border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${review.reported ? "bg-red-50/50 dark:bg-red-950/10" : ""}`}>
                  <td className="px-4 sm:px-6 py-3.5 font-medium text-slate-900 dark:text-white">{review.author}</td>
                  <td className="px-4 sm:px-6 py-3.5 text-slate-600 dark:text-slate-300 hidden sm:table-cell">{review.package}</td>
                  <td className="px-4 sm:px-6 py-3.5 text-slate-500 dark:text-slate-400 hidden lg:table-cell">{review.agency}</td>
                  <td className="px-4 sm:px-6 py-3.5"><StarRating rating={review.rating} /></td>
                  ...
                  <td className="px-4 sm:px-6 py-3.5">
                    {review.reported ? (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400"><Flag className="h-3 w-3" /> Flagged</span>
                    ) : (
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400">Valid</span>
                    )}
                  </td>
                  <td className="px-4 sm:px-6 py-3.5 text-slate-500 dark:text-slate-400 hidden lg:table-cell">{review.date}</td>
                  <td className="px-4 sm:px-6 py-3.5">
